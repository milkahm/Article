...
def insert_values(tbl, vals={}):
    ...
    for col, type in info:
        if col in vals or col.endswith("_id") or col == 'id':
            continue
...
def find_by_name(cls, tbl, name):
    ...
    conn.close()
    if record:
        return cls(**{desc[0]: record[desc[0]] for desc in row.description})

def count(tbl, where):
    conn = get_connection()
    cursor = conn.cursor()
    q = f"SELECT COUNT(*) AS count FROM {tbl} WHERE {list(where.keys())[0]} = ?"
...
def test_article_count_per_magazine():
    ...
    row = get_row("magazines")
    assert any(get_value(row, c, 'name') == "Tech Times" for c in counts)
